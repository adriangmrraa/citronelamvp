...
import CheckoutSummary from '@/components/features/market/CheckoutSummary';
...
export default function CheckoutPage() {
  ...
  const { products, isLoading: marketLoading } = useMarket();
  ...
  const handleConfirm = async () => {
    ...
    setTimeout(() => {
      const generatedId = spendTokens(product.price, `Canje de producto: ${product.name}`, product.id.toString());
      
      if (generatedId) {
        setTransactionId(generatedId);
        setFinalTotal(product.price);
              ...
              <div className="glass-surface border border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                <div className="p-8 border-b border-white/5 bg-white/[0.03] relative">
                   ...
                   <div className="flex gap-6 items-end">
                      <div className="w-20 h-20 relative rounded-2xl overflow-hidden border border-white/5 flex-shrink-0">
                         <img src={product.image} alt={product.name} className="object-cover w-full h-full" />
                      </div>
                      <div className="flex-1 space-y-1">
                        <p className="text-xl font-black text-white uppercase tracking-tight">{product.name}</p>
                      </div>
            ...
            <>
              ...
              <CheckoutTerminal onConfirm={handleConfirm} isProcessing={isProcessing}>
                <div className="space-y-6">
                    ...
                    <TokenStatus price={product.price} />
                </div>
              </CheckoutTerminal>
            </>
          )}
